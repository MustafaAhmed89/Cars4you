...
- **`packages/types/src/database.types.ts` is generated** from the hosted Supabase project via `pnpm db:types:remote` (`supabase gen types typescript --linked`; needs `SUPABASE_ACCESS_TOKEN` + a linked project — no Docker). It satisfies postgrest-js `GenericSchema`, with **every table AND view carrying a `Relationships` field** — omitting it on the view once made the whole client resolve to `never`. Now that it's generated with real FK relationships, the explicit `.returns<T>()`/cast workarounds in `api-client` for embedded selects (`listing_photos(...)`, `cities(name)`) can be simplified where desired. `pnpm db:types` (with `--local`) regenerates it from a local Docker DB instead, if one is running.
...
- **`node-linker=hoisted` is set in `pnpm-workspace.yaml`** (required for Expo/Metro under pnpm — otherwise `@expo/cli`/Metro can't resolve transitive deps like `metro-runtime`, and `expo start` fails). NOTE: pnpm 11 ignores the `node-linker` key in `.npmrc`, so it must live in `pnpm-workspace.yaml`; the leftover line in `.npmrc` is redundant/harmless.
